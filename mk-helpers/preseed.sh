...
provision() {
  if [ `docker-machine status $machine &> /dev/null; echo $?` -ne 0 ]; then
    ...
    case $1 in
      ...
      '-g' ) docker-machine create -d google --google-project $GOOGLE_PROJECT_ID --engine-install-url=https://releases.rancher.com/install-docker/17.05.sh --google-zone europe-west1-c --google-machine-type n1-standard-1 $machine;;
