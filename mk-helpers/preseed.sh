#!/usr/bin/env bash
# This script create docker-machine with Gitlab infrastructure

source mk-helpers/env.vars
provision() {
  if [ `docker-machine status $machine &> /dev/null; echo $?` -ne 0 ]; then
    printf "Создаем сервер с Gitlab CI\n"
    case $1 in
      ...
      '-g' ) docker-machine create -d google --google-project $GOOGLE_PROJECT_ID --engine-install-url=https://releases.rancher.com/install-docker/17.05.sh --google-zone europe-west1-c --google-machine-type n1-standard-2 $machine;;
...
seed() {
  ...
  printf "\nПодготовка проектов\n"
  docker-machine scp data/$BACKUP_FILE $machine:~/
  docker-machine ssh $machine "sudo cp ~/$BACKUP_FILE /srv/docker/gitlab/data/backups/"

  printf "\nИмпортируем проекты\n"
  docker-compose exec gitlab sh -c "yes yes | /opt/gitlab/bin/gitlab-rake gitlab:backup:restore BACKUP=$BACKUP" > /dev/null
...
create_images() {
  ...
  if ! docker pull $DOCKER_REGISTRY/module5/docker:git-compose &> /dev/null ; then
      ...
      if [[ "$OSTYPE" == "linux-gnu" ]]; then
        find . -type f -name Dockerfile -exec sed -i "s/docker:git/docker:1-git/" {} +
